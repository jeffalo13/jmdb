...
const re = (pat: string, flags = "i", raw = false) =>
  new RegExp(raw ? pat : `(?:^|\\b)${pat}(?:\\b|$)`, flags);

// ---------- Signal taxonomy ----------
// "Signals" are canonical concept tags we attach to many raw keywords.
// They are safer and more general than individual keywords, and the runtime
// rule engine maps Signals × Genres -> Flavors.
...
const SIGNALS_BY_FAMILY: Record<string, Signal[]> = {
  horror: [
...
const ALL_SIGNALS: Signal[] = Array.from(
  new Set(Object.values(SIGNALS_BY_FAMILY).flat())
).sort();

// ---------- Pattern rules (regex & exact) ----------
// Each entry gives a SIGNAL and a list of patterns that should map to that signal.
// Add pluralization / aliasing through regex, but keep them readable.
//
// NOTE: You can keep expanding this list over time; rebuilding will incorporate
// thousands of matching keywords automatically.

type SignalPattern = { signal: Signal; patterns: RegExp[]; alsoExact?: string[] };

const P: SignalPattern[] = [
  // Horror
...
type RawKeyword = { name?: string } | string;

const raw = JSON.parse(fs.readFileSync(INPUT, "utf8")) as RawKeyword[];
const names = new Set<string>();
for (const item of raw) {
  const rawName = typeof item === "string" ? item : (item?.name ?? "");
  const k = canonKeyword(rawName);
  if (!k || STOPWORDS.has(k)) continue;
  names.add(k);
}
...
const add = (kw: string, sig: Signal) => {
  ...
  if (!set) { set = new Set<Signal>(); mapKWtoSignals.set(kw, set); }
  set.add(sig);
};
