...
// function* expand(seed: string): Generator<string> {
//   const base = canon(seed);
//   yield base;

//   // hyphen/space toggles
//   if (base.includes("-")) yield base.replace(/-/g, " ");
//   if (base.includes(" ")) yield base.replace(/ /g, "-");
...
// const re = (pat: string, flags = "i") => new RegExp(`(?:^|\\b)${pat}(?:\\b|$)`, flags);

// // ---------- Signals taxonomy ----------
// // A "signal" is a normalized thematic/structural token that flavors can consume.
// // We keep this list broad so nearly any keyword can land somewhere.
// type Signal = string;
// type SignalPattern = {
//   signal: Signal;
//   // either phrases (auto-expanded) or explicit regexes:
//   phrases?: string[];
//   regexes?: RegExp[];
//   // optional genre hints (for smarter fallbacks)
...
// // === ACTION ===
// const ACTION_SEEDS: SignalPattern[] = [
...
// // === THRILLER ===
// const THRILLER_SEEDS: SignalPattern[] = [
...
// // === SCIENCE FICTION ===
// const SCIFI_SEEDS: SignalPattern[] = [
...
// // === FANTASY ===
// const FANTASY_SEEDS: SignalPattern[] = [
...
// // === HORROR ===
// const HORROR_SEEDS: SignalPattern[] = [
...
// // === CRIME / MYSTERY ===
// const CRIME_MYSTERY_SEEDS: SignalPattern[] = [
...
// // === ROMANCE / DRAMA ===
// const ROMANCE_DRAMA_SEEDS: SignalPattern[] = [
...
// // === COMEDY ===
// const COMEDY_SEEDS: SignalPattern[] = [
...
// const OTHER_SEEDS: SignalPattern[] = [
//   // History / War
...
// // ---------- Build giant regex table ----------
// type Compiled = { signal: Signal; tests: RegExp[] };
...
//         const clean = canon(v);
//         if (clean) tests.push(re(clean.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
//       }
//     }
//   }
//   if (row.regexes) tests.push(...row.regexes);
//   // de-dup tests
...
//     return true;
//   });
//   if (uniq.length) COMPILED.push({ signal: row.signal, tests: uniq });
// }
...
// const raw = JSON.parse(fs.readFileSync(INPUT, "utf8")) as Array<string | { name: string }>;
...
// // ---------- Match pass ----------
// const mapKWtoSignals = new Map<string, Set<Signal>>();
// const add = (kw: string, sig: Signal) => {
//   let s = mapKWtoSignals.get(kw);
//   if (!s) { s = new Set(); mapKWtoSignals.set(kw, s); }
//   s.add(sig);
// };
...
// const GENRE_HEURISTICS: Array<{ tokens: RegExp; signal: Signal }> = [
//   // Action-ish verbs
...
// for (const kw of KW) {
//   if (mapKWtoSignals.has(kw)) continue;
...
//   new Set(Array.from(mapKWtoSignals.values()).flatMap((s) => Array.from(s)))
// ).sort();

// const entries = Array.from(mapKWtoSignals.entries())
//   .map(([k, s]) => [k, Array.from(s).sort()] as const)
...
// for (const kw of KW) if (!mapKWtoSignals.has(kw)) unmatched.push(kw);
